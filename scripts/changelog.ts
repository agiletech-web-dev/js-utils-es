...
import md5 from 'md5';
import funcUtils from './funcUtils.json';
...
export async function getChangeLog(count = 200) {
  ...
  for (const log of logs) {
    ...
    log.functions = uniq(
      files
        .map((i) => {
          return /^src\/.*(?<!\.spec)\.ts$/.test(i) ? i : undefined;
        }) // Filter files that start with 'src' and end with '.ts'
        .filter(Boolean)
        .map((v: any) => {
          const name = v.split('/').pop()!.replace('.ts', '')
          return name;
        }),
    )
  }
...
export async function getFunctionContributors() {
  const result = await Promise.all(funcUtils.map(async (i) => {
    return [i.name, await getContributorsAt(`src/${i.package}/${i.name}`)] as const
  }))
